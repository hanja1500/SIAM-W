...
vulnerability_responses = {
    ...
    "stacked queries": "쿼리 작성 시 적절한 매개변수화를 수행하여 공격을 방어하세요.\nDBMS 설정을 변경하고, 입력값을 엄격하게 검증하여 보호하세요.",

    "boolean-based blind (time-based)": "시간에 따라 조건문의 참/거짓 여부를 파악하여 정보를 유출하는 공격에 대비하여 실행 시간 모니터링 및 입력값 검증을 강화하세요.\nORM 또는 Prepared Statement를 사용하여 쿼리를 작성하세요.",

    "union-based blind": "UNION 쿼리를 이용하여 정보를 추출하는 공격에 대비하여 UNION 쿼리 시 결과 열의 수를 일치시키세요.\nUNION 쿼리 대신 조인을 사용하여 데이터를 결합하여 공격 범위를 제한하는 것이 좋습니다.\n데이터베이스 사용자의 권한을 최소화하여 UNION 공격으로부터 보호하세요.",

    "error-based (boolean-based blind)": "에러 메시지를 이용하여 정보를 유출하는 공격에 대비하여 에러 메시지 노출을 최소화하고, 입력값을 엄격하게 검증하세요.\nORM 또는 Prepared Statement를 사용하여 쿼리를 작성하세요.",

    "inline query": "동적 쿼리를 생성하는 대신 ORM 또는 쿼리 빌더를 사용하여 정적 쿼리를 사용하여 쿼리의 안정성을 보장하세요.\nSQL Injection 대응을 위한 웹 애플리케이션 방화벽(WAF)을 구성하여 실시간으로 공격을 탐지 및 차단하세요.",
